...
/**
 * Fonte de letras para <a href="https://www.vagalume.com.br">vagalume.com.br</a>.
 *
 * <p>Estratégia de busca:</p>
 * <ol>
 *   <li>URL direta — {@code /{artist-slug}/{title-slug}.html}</li>
 *   <li>Página do artista — link com título exato</li>
 *   <li>Busca no site</li>
 * </ol>
 */
public class VagalumeSource extends AbstractLyricsSource {
    ...
    /**
     * Cria uma nova instância da fonte vagalume.com.br.
     *
     ...
     */
    public VagalumeSource(String baseUrl, String userAgent,
    ...
    /** {@inheritDoc} */
    @Override
    public String getName() { return "vagalume.com.br"; }
    ...
    /**
     * Busca a URL da letra no vagalume.com.br utilizando três estratégias:
     * URL direta, página do artista e busca no site.
     *
    ...
    /**
     * Extrai o texto da letra do elemento {@code div#lyrics} da página.
     *
...
//        }
    ...
    /**
     * Tenta URL direta: /{artist-slug}/{title-slug}.html
     *
     * @param artist nome do artista
     * @param title título da música
     * @return URL da página da letra, ou {@code null} se não encontrada
    ...
    private String findDirectUrl(String artist, String title) {
        ...
        for (String artistSlug : artistSlugs(artist)) {
            String url = getBaseUrl() + "/" + artistSlug + "/" + titleSlug + ".html";
    ...
    private String findOnArtistPage(String artist, String title) throws IOException {
        ...
        for (String artistSlug : artistSlugs(artist)) {
            ...
            Element link = artistDoc.select("a[href]").stream()
                    ...
                    .filter(a -> a.attr("href").endsWith(".html"))
    ...
    /**
     * Busca via endpoint de busca do site.
     *
     * @param artist nome do artista
     * @param title título da música
     * @return URL da página da letra, ou {@code null} se não encontrada
    ...
    private String extractSearchResult(Document doc, String title) {
        ...
        Element matchingLink = doc.select("a[href*='vagalume.com.br']").stream()
                .filter(a -> a.attr("href").endsWith(".html"))
                .filter(a -> toSlug(a.text()).equals(titleSlug))
                .findFirst().orElse(null);
        if (matchingLink != null) return matchingLink.attr("href");

        Element anyLink = doc.select("a[href*='vagalume.com.br']").stream()
                .filter(a -> a.attr("href").endsWith(".html"))
                .findFirst().orElse(null);
